...
void VnSearcher_dataReceivedHandler(void* portInfo)
{
	...
	struct VnPortInfo* port = (struct VnPortInfo*)portInfo;
	...
	enum VnError error = VnSerialPort_read(port->port, readBuffer, bytesToRead, &bytesRead);
...
void VnSearcher_testPort(void* portInfo)
{
	...
	while(index < numBaudrates)
	{
		...
		error = VnSerialPort_open_pnbr(&serialPort, port->portName, SubBD[index]);
...
char* VnSearcher_getPortName(struct VnPortInfo* portInfo)
{
	return portInfo->portName;
}

int32_t VnSearcher_getPortBaud(struct VnPortInfo* portInfo)
{
...
void VnSearcher_findAllSensors(struct VnPortInfo*** returnSensors, int32_t* numSensors)
{
...
void VnSearcher_searchInputPorts(char*** portsToCheck, int32_t numPortsToCheck, struct VnPortInfo*** sensorsFound)
{
	int32_t linkCount = 0;

	/* Initialize the sensorsFound array with space for any future pointers. */
	*sensorsFound = (struct VnPortInfo**) malloc(sizeof(struct VnPortInfo*) * numPortsToCheck);
	...
	while(linkCount < numPortsToCheck)
	{
		...
		(*sensorsFound)[linkCount] = (struct VnPortInfo*)malloc(sizeof(struct VnPortInfo));
