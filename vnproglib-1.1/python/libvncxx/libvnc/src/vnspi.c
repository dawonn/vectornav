...
#define SPI_CMD_MIN_SIZE  4

enum VnError
VnSpi_genGenericCommand(
    char cmdId,
    char* buffer,
    size_t* size,
...
enum VnError
VnSpi_genRestorFactorySettings(
    char* buffer,
    size_t* size,
    size_t desiredLength,
    size_t* responseSize)
    ...
    size_t desiredLength)
{
  ...
  if (*size < SPI_CMD_MIN_SIZE || *size < desiredLength)
    return E_BUFFER_TOO_SMALL;
  ...
  for (i = SPI_CMD_MIN_SIZE; i < desiredLength; i++)
    buffer[i] = 0x00;

  *size = desiredLength > SPI_CMD_MIN_SIZE ? desiredLength : SPI_CMD_MIN_SIZE;
    ...
    size_t tagLength)
{
  ...
  strcpy_x(tag, tagLength, pos);

  return E_NONE;
}

enum VnError
VnSpi_parseModelNumber(
    const char *response,
    char* productName,
    size_t productNameLength)
{
  ...
  strcpy_x(productName, productNameLength, pos);

  return E_NONE;
}

enum VnError
VnSpi_parseHardwareRevision(
    const char *response,
    uint32_t *revision)
{
  const char* pos = response + 3;
    ...
    size_t firmwareVersionLength)
{
  ...
  strcpy_x(firmwareVersion, firmwareVersionLength, pos);

  return E_NONE;
}

enum VnError
VnSpi_parseSerialBaudRate(
    const char *response,
    uint32_t *baudrate)
{
  const char* pos = response + 3;
...
enum VnError
VnSpi_parseAsyncDataOutputType(
    const char *response,
    uint32_t *ador)
{
  const char* pos = response + 3;
...
enum VnError
VnSpi_parseAsyncDataOutputFrequency(
    const char *response,
    uint32_t *adof)
{
  const char* pos = response + 3;
...
enum VnError
VnSpi_parseQuaternionMagneticAccelerationAndAngularRates(
    const char *response,
    union vn_vec4f *quat, 
    union vn_vec3f *mag, 
...
enum VnError
VnSpi_parseMagneticMeasurements(
    const char *response,
    union vn_vec3f *mag)
{
  const char* pos = response + 3;
...
enum VnError
VnSpi_parseAccelerationMeasurements(
    const char *response,
    union vn_vec3f *accel)
{
  const char* pos = response + 3;
...
enum VnError
VnSpi_parseAngularRateMeasurements(
    const char *response,
    union vn_vec3f *gyro)
{
  const char* pos = response + 3;
...
enum VnError
VnSpi_parseMagneticAccelerationAndAngularRates(
    const char *response,
    union vn_vec3f *mag, 
    union vn_vec3f *accel, 
    union vn_vec3f *gyro)
...
enum VnError
VnSpi_parseMagneticAndGravityReferenceVectors(
    const char *response,
...
enum VnError
VnSpi_parseFilterMeasurementsVarianceParameters(
    const char *response,
...
enum VnError
VnSpi_parseMagnetometerCompensation(
    const char *response,
    union vn_mat3f *c, 
    union vn_vec3f *b)
{
...
enum VnError
VnSpi_parseFilterActiveTuningParameters(
    const char *response,
...
enum VnError
VnSpi_parseAccelerationCompensation(
    const char *response,
    union vn_mat3f *c, 
    union vn_vec3f *b)
{
...
enum VnError
VnSpi_parseReferenceFrameRotation(
    const char *response,
    union vn_mat3f *c)
{
  const char* pos = response + 3;
...
enum VnError
VnSpi_parseYawPitchRollMagneticAccelerationAndAngularRates(
    const char *response,
    union vn_vec3f *yawPitchRoll, 
    union vn_vec3f *mag, 
    union vn_vec3f *accel, 
...
enum VnError
VnSpi_parseCommunicationProtocolControl(
    const char *response,
    uint8_t *serialCount, 
...
enum VnError
VnSpi_parseSynchronizationControl(
    const char *response,
    uint8_t *syncInMode, 
    uint8_t *syncInEdge, 
...
enum VnError
VnSpi_parseSynchronizationStatus(
    const char *response,
...
enum VnError
VnSpi_parseVpeMagnetometerBasicTuning(
    const char *response,
...
enum VnError
VnSpi_parseVpeMagnetometerAdvancedTuning(
    const char *response,
...
enum VnError
VnSpi_parseVpeAccelerometerBasicTuning(
    const char *response,
    union vn_vec3f *baseTuning, 
    union vn_vec3f *adaptiveTuning, 
    union vn_vec3f *adaptiveFiltering)
...
enum VnError
VnSpi_parseVpeAccelerometerAdvancedTuning(
    const char *response,
    union vn_vec3f *minFiltering, 
    union vn_vec3f *maxFiltering, 
    float *maxAdaptRate, 
...
enum VnError
VnSpi_parseFilterStartupGyroBias(
    const char *response,
    union vn_vec3f *bias)
{
  const char* pos = response + 3;
...
enum VnError
VnSpi_parseMagnetometerCalibrationControl(
    const char *response,
    uint8_t *hsiMode, 
    uint8_t *hsiOutput, 
...
enum VnError
VnSpi_parseCalculatedMagnetometerCalibration(
    const char *response,
    union vn_mat3f *c, 
    union vn_vec3f *b)
{
...
enum VnError
VnSpi_parseIndoorHeadingModeControl(
    const char *response,
    float *maxRateError, 
    uint8_t *reserved1)
{
...
enum VnError
VnSpi_parseVelocityCompensationMeasurement(
    const char *response,
...
enum VnError
VnSpi_parseVelocityCompensationControl(
    const char *response,
    uint8_t *mode, 
...
enum VnError
VnSpi_parseVelocityCompensationStatus(
    const char *response,
    float *x, 
    float *xDot, 
    ...
    float *velUncertainty)
{
  ...
  *week = VnUtil_extractUint16(pos);
  pos += sizeof(uint16_t);
  *status = VnUtil_extractUint16(pos);
  pos += sizeof(uint16_t);
  *yawPitchRoll = VnUtil_extractVec3f(pos);
  pos += 3 * sizeof(float);
  *position = VnUtil_extractVec3d(pos);
...
enum VnError
VnSpi_parseInsBasicConfiguration(
    const char *response,
    uint8_t *scenario, 
    uint8_t *ahrsAiding, 
    uint8_t *estBaseline, 
...
enum VnError
VnSpi_parseInsAdvancedConfiguration(
    const char *response,
    uint8_t *useMag, 
    uint8_t *usePres, 
    uint8_t *posAtt, 
...
enum VnError
VnSpi_parseStartupFilterBiasEstimate(
    const char *response,
...
enum VnError
VnSpi_parseDeltaThetaAndDeltaVelocity(
    const char *response,
    float *deltaTime, 
...
enum VnError
VnSpi_parseDeltaThetaAndDeltaVelocityConfiguration(
    const char *response,
...
enum VnError
VnSpi_parseReferenceVectorConfiguration(
    const char *response,
    uint8_t *useMagModel, 
...
enum VnError
VnSpi_parseImuFilteringConfiguration(
    const char *response,
...
enum VnError
VnSpi_parseGpsCompassEstimatedBaseline(
    const char *response,
...
enum VnError
VnSpi_parseImuRateConfiguration(
    const char *response,
    uint16_t *imuRate, 
    uint16_t *navDivisor, 
...
enum VnError
VnSpi_parseYawPitchRollTrueBodyAccelerationAndAngularRates(
    const char *response,
    union vn_vec3f *yawPitchRoll, 
...
enum VnError
VnSpi_parseYawPitchRollTrueInertialAccelerationAndAngularRates(
    const char *response,
    union vn_vec3f *yawPitchRoll, 
...
enum VnError VnSpi_genReadUserTag(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 4;
...
enum VnError VnSpi_genReadModelNumber(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 4;
...
enum VnError VnSpi_genReadHardwareRevision(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 8;
...
enum VnError VnSpi_genReadSerialNumber(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 8;
...
enum VnError VnSpi_genReadFirmwareVersion(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 4;
...
enum VnError VnSpi_genReadSerialBaudRate(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 9;
...
enum VnError VnSpi_genReadAsyncDataOutputType(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 9;
...
enum VnError VnSpi_genReadAsyncDataOutputFrequency(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 9;
...
enum VnError VnSpi_genReadYawPitchRoll(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 16;
...
enum VnError VnSpi_genReadAttitudeQuaternion(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 20;
...
enum VnError VnSpi_genReadQuaternionMagneticAccelerationAndAngularRates(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 56;
...
enum VnError VnSpi_genReadMagneticMeasurements(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 16;
...
enum VnError VnSpi_genReadAccelerationMeasurements(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 16;
...
enum VnError VnSpi_genReadAngularRateMeasurements(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 16;
...
enum VnError VnSpi_genReadMagneticAccelerationAndAngularRates(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 40;
...
enum VnError VnSpi_genReadMagneticAndGravityReferenceVectors(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 28;
...
enum VnError VnSpi_genReadMagnetometerCompensation(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 52;
...
enum VnError VnSpi_genReadAccelerationCompensation(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 52;
...
enum VnError VnSpi_genReadReferenceFrameRotation(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 40;
...
enum VnError VnSpi_genReadYawPitchRollMagneticAccelerationAndAngularRates(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 52;
...
enum VnError VnSpi_genReadCommunicationProtocolControl(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 11;
...
enum VnError VnSpi_genReadSynchronizationControl(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 24;
...
enum VnError VnSpi_genReadSynchronizationStatus(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 16;
...
enum VnError VnSpi_genReadVpeBasicControl(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 8;
...
enum VnError VnSpi_genReadVpeMagnetometerBasicTuning(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 40;
...
enum VnError VnSpi_genReadVpeAccelerometerBasicTuning(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 40;
...
enum VnError VnSpi_genReadMagnetometerCalibrationControl(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 7;
...
enum VnError VnSpi_genReadCalculatedMagnetometerCalibration(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 52;
...
enum VnError VnSpi_genReadVelocityCompensationMeasurement(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 16;
...
enum VnError VnSpi_genReadVelocityCompensationControl(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 13;
...
enum VnError VnSpi_genReadImuMeasurements(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 48;
...
enum VnError VnSpi_genReadGpsConfiguration(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 9;
...
enum VnError VnSpi_genReadGpsAntennaOffset(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 16;
...
enum VnError VnSpi_genReadGpsSolutionLla(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 76;
...
enum VnError VnSpi_genReadGpsSolutionEcef(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 76;
...
enum VnError VnSpi_genReadInsSolutionLla(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 76;
...
enum VnError VnSpi_genReadInsSolutionEcef(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 76;
...
enum VnError VnSpi_genReadInsBasicConfiguration(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 8;
...
enum VnError VnSpi_genReadInsStateLla(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 76;
...
enum VnError VnSpi_genReadInsStateEcef(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 76;
...
enum VnError VnSpi_genReadStartupFilterBiasEstimate(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 32;
...
enum VnError VnSpi_genReadDeltaThetaAndDeltaVelocity(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 32;
...
enum VnError VnSpi_genReadDeltaThetaAndDeltaVelocityConfiguration(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 10;
...
enum VnError VnSpi_genReadReferenceVectorConfiguration(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 44;
...
enum VnError VnSpi_genReadGyroCompensation(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 52;
...
enum VnError VnSpi_genReadImuFilteringConfiguration(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 19;
...
enum VnError VnSpi_genReadGpsCompassBaseline(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 28;
...
enum VnError VnSpi_genReadGpsCompassEstimatedBaseline(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 32;
...
enum VnError VnSpi_genReadYawPitchRollTrueBodyAccelerationAndAngularRates(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 40;
...
enum VnError VnSpi_genReadYawPitchRollTrueInertialAccelerationAndAngularRates(char* buffer, size_t* size, size_t desiredLength, size_t* responseSize)
{
	*responseSize = 40;
...
enum VnError
VnSpi_genWriteAsyncDataOutputFrequency(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
...
enum VnError
VnSpi_genWriteMagneticAndGravityReferenceVectors(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
    ...
    union vn_vec3f accRef)
{
  ...
  *pos++ = 0;
  ...
  memcpy(pos, &magRef, sizeof(union vn_vec3f));
  pos += sizeof(union vn_vec3f);
  accRef.c[0] = htosf4(accRef.c[0]);
  accRef.c[1] = htosf4(accRef.c[1]);
  accRef.c[2] = htosf4(accRef.c[2]);
  memcpy(pos, &accRef, sizeof(union vn_vec3f));
  pos += sizeof(union vn_vec3f);
...
enum VnError
VnSpi_genWriteMagnetometerCompensation(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
    ...
    union vn_vec3f b)
{
  ...
  *pos++ = 0;
  ...
  c.e[7] = htosf4(c.e[7]);
  c.e[8] = htosf4(c.e[8]);
  memcpy(pos, &c, sizeof(union vn_mat3f));
  pos += sizeof(union vn_mat3f);
  b.c[0] = htosf4(b.c[0]);
  b.c[1] = htosf4(b.c[1]);
  b.c[2] = htosf4(b.c[2]);
  memcpy(pos, &b, sizeof(union vn_vec3f));
  pos += sizeof(union vn_vec3f);
...
enum VnError
VnSpi_genWriteAccelerationCompensation(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
...
enum VnError
VnSpi_genWriteCommunicationProtocolControl(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
...
enum VnError
VnSpi_genWriteSynchronizationControl(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
...
enum VnError
VnSpi_genWriteSynchronizationStatus(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
    ...
    union vn_vec3f adaptiveFiltering)
{
  ...
  *pos++ = 0;
  ...
  memcpy(pos, &baseTuning, sizeof(union vn_vec3f));
  pos += sizeof(union vn_vec3f);
  ...
  memcpy(pos, &adaptiveTuning, sizeof(union vn_vec3f));
  pos += sizeof(union vn_vec3f);
  ...
  memcpy(pos, &adaptiveFiltering, sizeof(union vn_vec3f));
  pos += sizeof(union vn_vec3f);
...
enum VnError
VnSpi_genWriteVpeAccelerometerBasicTuning(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
...
enum VnError
VnSpi_genWriteMagnetometerCalibrationControl(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
...
enum VnError
VnSpi_genWriteVelocityCompensationMeasurement(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
    union vn_vec3f velocity)
{
  ...
  *pos++ = 0;
  ...
  memcpy(pos, &velocity, sizeof(union vn_vec3f));
  pos += sizeof(union vn_vec3f);
...
enum VnError
VnSpi_genWriteVelocityCompensationControl(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
...
enum VnError
VnSpi_genWriteGpsAntennaOffset(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
    union vn_vec3f position)
{
  ...
  *pos++ = 0;
  ...
  memcpy(pos, &position, sizeof(union vn_vec3f));
  pos += sizeof(union vn_vec3f);
...
enum VnError
VnSpi_genWriteInsBasicConfiguration(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
...
enum VnError
VnSpi_genWriteStartupFilterBiasEstimate(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
...
enum VnError
VnSpi_genWriteDeltaThetaAndDeltaVelocityConfiguration(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
...
enum VnError
VnSpi_genWriteReferenceVectorConfiguration(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
    ...
    union vn_vec3d position)
{
  ...
  *pos++ = resv2;
  ...
  memcpy(pos, &position, sizeof(union vn_vec3d));
  pos += sizeof(union vn_vec3d);

  return E_NONE;
}

enum VnError
VnSpi_genWriteGyroCompensation(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
...
enum VnError
VnSpi_genWriteImuFilteringConfiguration(
    char *buffer,
    size_t *size,
    size_t desiredLength,
    size_t *responseSize,
    ...
    union vn_vec3f uncertainty)
{
  ...
  *pos++ = 0;
  ...
  memcpy(pos, &uncertainty, sizeof(union vn_vec3f));
  pos += sizeof(union vn_vec3f);
