...
template <size_t tdim, typename T> std::string to_string(vec<tdim, T> v)
{
	std::stringstream ss;
	ss << "(";
...
/// \brief Parses a string that represents a vector.
/// \param[in] in The string to parse.
/// \paramt[out] v The vector to hold the parsed value.
/// \return Indicates if the parse was successful.
template <size_t tdim>
bool parse(const std::string &in, vec<tdim, float> &v)
{
...
/// \return Indicates if the parse was successful.
template <size_t tdim>
bool parse(const std::string &in, vec<tdim, double> &v)
{
  char *token = strtok(const_cast<char*>(in.c_str()), "(;) ");
