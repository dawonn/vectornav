...
/// \brief Converts a value to a binary string representation.
/// \param[in] v The value to convert.
/// \return The binary string representation.
std::string to_binary_string(uint16_t v);

/// \brief Parses a binary string in the form "00010010".
/// \param[in] s The string to parse.
/// \return The resulting value.
uint32_t parse_binary_string(const std::string& s);

/// \brief Splits a string based on provided deliminators.
/// \param[in] s The string to split.
/// \param[in] delims The list of deliminators to use.
/// \param[in] removeEmpties Remove empty strings from the returned list.
/// \return The split items.
std::vector<std::string> split(const std::string &s, const std::string &delims = " ", bool removeEmpties = true);
