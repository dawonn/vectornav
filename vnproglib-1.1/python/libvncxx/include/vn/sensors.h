...
public:
	...
	static std::vector<uint32_t> supportedBaudrates();
  	...
  	/// \brief Indicates the supported frequencies for async data output.
  	static std::vector<uint32_t> supportedAsyncDataOutputFrequencies();
  ...
  /// \param[in] changeSensorAlso Indicates to change the sensor's own baudrate first.
  /// \param[in] activeSensorPort Indicates which serial port is the active communication
  ///     channel for the sensor. If 0, will attempt to determine which port is active if
  ///     sensor supports this feature. Otherwise, it will assume SerialPort1 is active. If
  ///     1, SerialPort1 is assumed active. If 2, SerialPort2 is assumed active.
	void changeBaudRate(uint32_t baudrate, bool changeSensorAlso = true, uint8_t activeSensorPort = 0);
	...
	/// \brief Allows registering of a hander to receive notification if the user unplugs the sensor's USB cable.
	/// \param[in] userData Pointer to user data, which will be provided to the callback method.
	/// \param[in] handler The callback method.
	virtual void registerUsbCableUnpluggedNotificationHandler(void* userData, void (*handler)(void*));

	/// \brief Unreigsters the callback method.
	virtual void unregisterUsbCableUnpluggedNotificationHandler();
  ...
  /// \defgroup vnSensorFeatures VectorNav Sensor Features
  /// @{


  //std::vector<vn::protocol::uart::AsciiAsync> supportedAsyncOutputDataTypes();
