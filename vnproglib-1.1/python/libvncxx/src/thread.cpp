...
bool NewThread::isFinished()
{
	return m_isFinished;
}

bool NewThread::isRunning()
{
    return m_isRunning;
}
...
void NewThread::runThread(void* aThread)
{
    static_cast<NewThread*>(aThread)->run();
}
...
void NewThread::sleepSec(uint32_t numOfSecsToSleep)
{
#if _WIN32
...
void NewThread::sleepMs(uint32_t numOfMsToSleep)
{
#if _WIN32
