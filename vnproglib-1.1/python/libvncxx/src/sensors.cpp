...
#if _NEW_SERIAL_PORT
struct VnSensor::Impl : public NewThread
#else
...
#endif
{
...
#if _NEW_SERIAL_PORT
	NewSerialPort* pNSP;
	CriticalSection _packetStoreCS;
#else
	...
	void run()
	{
		...
		while (m_isRunning)
		{
			// Process packets only while the serial port is not finished running
			// and one is available.
			if((NULL != pNSP) &&
				!pNSP->isFinished() &&
				pNSP->hasPacket())
			{
				packet = pNSP->getNextPacket();

				if (_waitingForResponse && packet->isResponse())
				{
					_transactionCS.enter();
					_receivedResponses.push(*packet);
					_transactionCS.leave();
				}
				else if(packet->isAsciiAsync())
				{
					onAsyncPacketReceived(*packet, packetCounter++, TimeStamp::get());
				}

				// Do clean up because receiving functions take in a copy.
				delete packet;
			}
			else
			{
	...
	Packet transactionWithWait(char* toSend, size_t length, uint16_t responseTimeoutMs, uint16_t retransmitDelayMs)
	{
		...
		while (true)
		{
			...
			while (!responsesToProcess.empty())
			{
				...
				{
					_waitingForResponse = false;
					throw devices::device_error(p.parseError());
				}
	...
	void transactionNoFinalize(char* toSend, size_t length, bool waitForReply, Packet *response, uint16_t responseTimeoutMs, uint16_t retransmitDelayMs)
	{
		...
		if (waitForReply)
		{
			*response = transactionWithWait(toSend, length, responseTimeoutMs, retransmitDelayMs);
			...
			if (response->isError())
				throw devices::device_error(response->parseError());
		}
		else
		{
			port->write(toSend, length);
...
std::vector<uint32_t> VnSensor::supportedAsyncDataOutputFrequencies()
{
	uint32_t freqs[] = {
		1,
		2,
	...
	return vector<uint32_t>(freqs, freqs + sizeof(freqs) / sizeof(uint32_t));
}
...
void VnSensor::registerUsbCableUnpluggedNotificationHandler(void *userData, void (*handler)(void *))
{
	if (!_pi->SimplePortIsOurs)
		throw invalid_operation();

	_pi->pSerialPort->registerUsbCableUnpluggedNotificationHandler(userData, handler);
}

void VnSensor::unregisterUsbCableUnpluggedNotificationHandler()
{
	if (!_pi->SimplePortIsOurs)
		throw invalid_operation();

	_pi->pSerialPort->unregisterUsbCableUnpluggedNotificationHandler();
}
...
void VnSensor::changeBaudRate(uint32_t baudrate, bool changeSensorAlso, uint8_t activeSensorPort)
{
	if (changeSensorAlso)
	{
    if (activeSensorPort == 0)
      // TODO: Add code to check which serial port is active on the sensor when this feature is available.
      writeSerialBaudRate(baudrate, true);
    else if (activeSensorPort == 1)
      writeSerialBaudRate(baudrate, 1, true);
    else if (activeSensorPort == 2)
      writeSerialBaudRate(baudrate, 2, true);
    else
      throw invalid_argument("invalid value for activeSensorPort");
	}
