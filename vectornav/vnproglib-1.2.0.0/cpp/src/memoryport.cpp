...
MemoryPort::MemoryPort() : _pi(new Impl(this)) {}

#if defined(_MSC_VER)
#pragma warning(pop)
#endif

MemoryPort::~MemoryPort() { delete _pi; }

void MemoryPort::open()
{
  if (_pi->IsOpen) throw invalid_operation();
...
void MemoryPort::read(char dataBuffer[], size_t numOfBytesToRead, size_t & numOfBytesActuallyRead)
{
  ...
  copy(
    _pi->DataAvailableForRead, _pi->DataAvailableForRead + _pi->DataAvailableForReadLength,
    dataBuffer);
...
{
  if (_pi->_dataWrittenHandler != NULL) throw invalid_operation();
...
void MemoryPort::SendDataBackDoor(const char data[], std::size_t length)
{
  SendDataBackDoor(reinterpret_cast<const uint8_t *>(data), length);
}

void MemoryPort::SendDataBackDoor(const std::string data)
{
