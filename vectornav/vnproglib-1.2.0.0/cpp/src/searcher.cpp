...
struct TestHelper
{
  ...
  : serialPort(serialPort), packetFinder(packetFinder)
  {
  }
};
...
struct SearchHelper
{
  ...
  : sensorFound(false), portName(portName), foundBaudrate(0)
  {
  }
};

void testDataReceivedHandler(void * userData);
void testValidPacketFoundHandler(
  void * userData, Packet & packet, size_t runningIndexOfPacketStart, TimeStamp timestamp);
...
{
#if __cplusplus < 201103L
  vector<uint32_t> TestBaudrates(
    TestBaudratesRaw, TestBaudratesRaw + sizeof(TestBaudratesRaw) / sizeof(TestBaudratesRaw[0]));
#endif
  for (vector<uint32_t>::const_iterator it = TestBaudrates.begin(); it != TestBaudrates.end();
       ++it) {
