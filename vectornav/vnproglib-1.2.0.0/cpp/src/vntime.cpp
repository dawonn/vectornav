...
#error "Unknown System"
  ...
  float elapsedMs()
  {
    ...
    if (_counterStart == -1) throw unknown_error();
...
Stopwatch::Stopwatch() : _pi(new Impl()) {}

Stopwatch::~Stopwatch() { delete _pi; }

void Stopwatch::reset() { _pi->reset(); }

float Stopwatch::elapsedMs() { return _pi->elapsedMs(); }
