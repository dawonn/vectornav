...
void EzAsyncData::asyncPacketReceivedHandler(
  void * userData, protocol::uart::Packet & p, size_t index)
{
...
EzAsyncData::EzAsyncData(VnSensor * sensor) : _sensor(sensor)
{
  _sensor->registerAsyncPacketReceivedHandler(this, &EzAsyncData::asyncPacketReceivedHandler);
}

VnSensor * EzAsyncData::sensor() { return _sensor; }
...
CompositeData EzAsyncData::getNextData() { return getNextData(0xFFFFFFFF); }

CompositeData EzAsyncData::getNextData(int timeoutMs)
{
  if (_newDataEvent.waitMs(timeoutMs) == xplat::Event::WAIT_TIMEDOUT) throw timeout();
