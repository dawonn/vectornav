...
Thread::Thread(ThreadStartRoutine startRoutine) : _pimpl(new Thread::Impl())
{
  _pimpl->StartRoutine = startRoutine;
}

Thread::~Thread() { delete _pimpl; }
...
void Thread::start(void * routineData)
{
  if (_pimpl->StartRoutine != NULL) {
    ...
    _pimpl->ThreadHandle =
      CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)_pimpl->StartRoutine, routineData, 0, NULL);

    if (_pimpl->ThreadHandle == NULL) throw unknown_error();

#elif __linux__ || __APPLE__ || __CYGWIN__ || __QNXNTO__

    _pimpl->Data = routineData;

    int errorCode = pthread_create(&_pimpl->ThreadHandle, NULL, Impl::StartRoutineWrapper, _pimpl);

    if (errorCode != 0) throw unknown_error();
...
void Thread::join()
{
  ...
  WaitForSingleObject(_pimpl->ThreadHandle, INFINITE);

#elif __linux__ || __APPLE__ || __CYGWIN__ || __QNXNTO__

  if (pthread_join(_pimpl->ThreadHandle, NULL)) throw unknown_error();
