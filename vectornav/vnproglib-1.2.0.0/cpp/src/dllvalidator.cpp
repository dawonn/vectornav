...
bool DllValidator::initialize()
{
  ...
  if (mPeFile != NULL) {
    if ((mPeFile->readMzHeader() == 0) && (mPeFile->readPeHeader() == 0)) {
      mIsInitialized = true;
    }
  }
...
bool DllValidator::hasDllNames() { return mVisitor.mRequiredDlls.size() > 0; }
...
bool DllValidator::validate()
{
  ...
  if (mIsInitialized) {
    ...
    for (size_t index = 0; index < mVisitor.mRequiredDlls.size(); index++) {
      HMODULE module =
        LoadLibraryEx(mVisitor.mRequiredDlls[index].c_str(), NULL, LOAD_LIBRARY_AS_IMAGE_RESOURCE);
      if (NULL == module) {
