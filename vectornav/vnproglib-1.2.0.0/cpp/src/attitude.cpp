...
AttitudeF AttitudeF::fromQuat(vec4f quat) { return AttitudeF(ATT_Quat, &quat); }

AttitudeF AttitudeF::fromYprInDegs(vec3f yprInDegs) { return AttitudeF(ATT_YprDegs, &yprInDegs); }

AttitudeF AttitudeF::fromYprInRads(vec3f yprInRads) { return AttitudeF(ATT_YprRads, &yprInRads); }

AttitudeF AttitudeF::fromDcm(mat3f dcm) { return AttitudeF(ATT_Dcm, &dcm); }

AttitudeF::AttitudeF(AttitudeType type, void * attitude) : _underlyingType(type)
{
  ...
  copy(
    static_cast<uint8_t *>(attitude), static_cast<uint8_t *>(attitude) + numOfBytesToCopy, _data);
}
