...
sensor_error::sensor_error(SensorError e) : exception(), error(e), _errorMessage(NULL) {}

sensor_error::sensor_error(sensor_error const & e)
: exception(), error(e.error), _errorMessage(NULL)
{
}

sensor_error::~sensor_error() throw()
{
  if (_errorMessage != NULL) delete[] _errorMessage;
}
...
struct VnSensor::Impl
{
  ...
  ~Impl() { _packetFinder.unregisterPossiblePacketFoundHandler(); }

  void onPossiblePacketFound(Packet & possiblePacket, size_t packetStartRunningIndex)
  {
    if (_possiblePacketFoundHandler != NULL)
      _possiblePacketFoundHandler(
        _possiblePacketFoundUserData, possiblePacket, packetStartRunningIndex);
  }
  ...
  static void possiblePacketFoundHandler(
    void * userData, Packet & possiblePacket, size_t packetStartRunningIndex, TimeStamp timestamp)
  {
  ...
  static void dataReceivedHandler(void * userData)
  {
    ...
    pi->port->read(pi->readBuffer, DefaultReadBufferSize, numOfBytesRead);
    ...
    if (pi->_rawDataReceivedHandler != NULL)
      pi->_rawDataReceivedHandler(
        pi->_rawDataReceivedUserData, reinterpret_cast<char *>(pi->readBuffer), numOfBytesRead,
        pi->_dataRunningIndex);
    ...
    pi->_packetFinder.processReceivedData(
      reinterpret_cast<char *>(pi->readBuffer), numOfBytesRead, pi->_filteringBootloaderResponses,
      t);

    pi->_dataRunningIndex += numOfBytesRead;
  }

  bool isConnected() { return port != NULL && port->isOpen(); }
...
#pragma warning(pop)
  ...
  Packet transactionWithWait(
    char * toSend, size_t length, uint16_t responseTimeoutMs, uint16_t retransmitDelayMs)
  {
    ...
    while (true) {
      ...
      xplat::Event::WaitResult waitResult =
        _newResponsesEvent.waitUs(static_cast<uint32_t>(responseWaitTime * 1000));
  ...
  void transactionNoFinalize(
    char * toSend, size_t length, bool waitForReply, Packet * response, uint16_t responseTimeoutMs,
    uint16_t retransmitDelayMs)
  {
    if (!isConnected()) throw invalid_operation();

    if (waitForReply) {
      *response = transactionWithWait(toSend, length, responseTimeoutMs, retransmitDelayMs);

      if (response->isError()) throw sensor_error(response->parseError());
    } else {
      port->write(toSend, length);
    }
  }
  ...
  {
    transactionNoFinalize(
      toSend, length, waitForReply, response, _responseTimeoutMs, _retransmitDelayMs);
  }
    ...
    uint16_t retransmitDelayMs)
  {
    ...
    transactionNoFinalize(
      toSend, length, waitForReply, response, responseTimeoutMs, retransmitDelayMs);
  }
  ...
  BinaryOutputRegister readBinaryOutput(uint8_t binaryOutputNumber)
  {
    char toSend[17];
    Packet response;
    uint16_t asyncMode, rateDivisor, outputGroup, commonField, timeField, imuField, gpsField,
      attitudeField, insField, gps2Field;
    ...
    response.parseBinaryOutput(
      &asyncMode, &rateDivisor, &outputGroup, &commonField, &timeField, &imuField, &gpsField,
      &attitudeField, &insField, &gps2Field);

    return BinaryOutputRegister(
      static_cast<AsyncMode>(asyncMode), rateDivisor, static_cast<CommonGroup>(commonField),
      static_cast<TimeGroup>(timeField), static_cast<ImuGroup>(imuField),
      static_cast<GpsGroup>(gpsField), static_cast<AttitudeGroup>(attitudeField),
      static_cast<InsGroup>(insField), static_cast<GpsGroup>(gps2Field));
  }

  void writeBinaryOutput(
    uint8_t binaryOutputNumber, BinaryOutputRegister & fields, bool waitForReply)
  {
    char toSend[256];
    Packet response;
    ...
    uint16_t groups = 0;
    if (fields.commonField) groups |= 0x0001;
    if (fields.timeField) groups |= 0x0002;
    if (fields.imuField) groups |= 0x0004;
    if (fields.gpsField) groups |= 0x0008;
    if (fields.attitudeField) groups |= 0x0010;
    if (fields.insField) groups |= 0x0020;
    if (fields.gps2Field) groups |= 0x0040;

#if VN_HAVE_SECURE_CRT
    int length = sprintf_s(
      toSend, sizeof(toSend), "$VNWRG,%u,%u,%u,%X", 74 + binaryOutputNumber, fields.asyncMode,
      fields.rateDivisor, groups);
#else
    int length = sprintf(
      toSend, "$VNWRG,%u,%u,%u,%X", 74 + binaryOutputNumber, fields.asyncMode, fields.rateDivisor,
      groups);
#endif
...
{
  uint32_t br[] = {9600, 19200, 38400, 57600, 115200, 128000, 230400, 460800, 921600};
...
VnSensor::VnSensor() : _pi(new Impl(this))
{
...
{
  if (_pi != NULL) {
    if (_pi->SimplePortIsOurs && _pi->DidWeOpenSimplePort && isConnected()) disconnect();
...
ErrorDetectionMode VnSensor::sendErrorDetectionMode() { return _pi->_sendErrorDetectionMode; }
...
bool VnSensor::isConnected() { return _pi->isConnected(); }

uint16_t VnSensor::responseTimeoutMs() { return _pi->_responseTimeoutMs; }

void VnSensor::setResponseTimeoutMs(uint16_t timeout) { _pi->_responseTimeoutMs = timeout; }

uint16_t VnSensor::retransmitDelayMs() { return _pi->_retransmitDelayMs; }

void VnSensor::setRetransmitDelayMs(uint16_t delay) { _pi->_retransmitDelayMs = delay; }
...
{
  if (_pi->port == NULL || !_pi->port->isOpen()) throw invalid_operation();
...
string VnSensor::send(string toSend, bool waitForReply, ErrorDetectionMode errorDetectionMode)
{
...
#if VN_HAVE_SECURE_CRT
      curToSendLength += sprintf_s(
        buffer + curToSendLength, toSend.size() + 8 - curToSendLength, "%02X\r\n",
        Checksum8::compute(buffer + 1, curToSendLength - 2));
#else
      curToSendLength += sprintf(
        buffer + curToSendLength, "%02X\r\n", Checksum8::compute(buffer + 1, curToSendLength - 2));
#endif
    } else if (errorDetectionMode == ERRORDETECTIONMODE_CRC) {
#if VN_HAVE_SECURE_CRT
      curToSendLength += sprintf_s(
        buffer + curToSendLength, toSend.size() + 8 - curToSendLength, "%04X\r\n",
        Crc16::compute(buffer + 1, curToSendLength - 2));
#else
      curToSendLength += sprintf(
        buffer + curToSendLength, "%04X\r\n", Crc16::compute(buffer + 1, curToSendLength - 2));
#endif
    } else {
#if VN_HAVE_SECURE_CRT
      curToSendLength +=
        sprintf_s(buffer + curToSendLength, toSend.size() + 8 - curToSendLength, "*XX\r\n");
#else
  ...
  _pi->transactionNoFinalize(
    buffer, curToSendLength, waitForReply, &p, _pi->_responseTimeoutMs, _pi->_retransmitDelayMs);
...
{
  if (_pi->_rawDataReceivedHandler != NULL) throw invalid_operation();
...
void VnSensor::registerPossiblePacketFoundHandler(
  void * userData, PossiblePacketFoundHandler handler)
{
  if (_pi->_possiblePacketFoundHandler != NULL) throw invalid_operation();
...
void VnSensor::registerAsyncPacketReceivedHandler(
  void * userData, AsyncPacketReceivedHandler handler)
{
  if (_pi->_asyncPacketReceivedHandler != NULL) throw invalid_operation();
...
void VnSensor::registerErrorPacketReceivedHandler(
  void * userData, ErrorPacketReceivedHandler handler)
{
  if (_pi->_errorPacketReceivedHandler != NULL) throw invalid_operation();
...
void VnSensor::magneticDisturbancePresent(bool disturbancePresent, bool waitForReply)
{
  ...
  size_t length = Packet::genKnownMagneticDisturbance(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), disturbancePresent);
...
void VnSensor::accelerationDisturbancePresent(bool disturbancePresent, bool waitForReply)
{
  ...
  size_t length = Packet::genKnownAccelerationDisturbance(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), disturbancePresent);
...
void VnSensor::restoreFactorySettings(bool waitForReply)
{
  ...
  size_t length =
    Packet::genRestoreFactorySettings(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
{
  if (modelNumber.find("VN-100") == 0) return VnSensor_Family_Vn100;

  if (modelNumber.find("VN-200") == 0) return VnSensor_Family_Vn200;

  if (modelNumber.find("VN-300") == 0) return VnSensor_Family_Vn300;

  return VnSensor_Family_Unknown;
}

BinaryOutputRegister VnSensor::readBinaryOutput1() { return _pi->readBinaryOutput(1); }

BinaryOutputRegister VnSensor::readBinaryOutput2() { return _pi->readBinaryOutput(2); }

BinaryOutputRegister VnSensor::readBinaryOutput3() { return _pi->readBinaryOutput(3); }
...
uint32_t VnSensor::readSerialBaudRate(uint8_t port)
{
  ...
  size_t length =
    Packet::genReadSerialBaudRate(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend), port);
...
void VnSensor::writeSerialBaudRate(const uint32_t & baudrate, uint8_t port, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteSerialBaudRate(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), baudrate, port);
...
AsciiAsync VnSensor::readAsyncDataOutputType(uint8_t port)
{
  ...
  size_t length =
    Packet::genReadAsyncDataOutputType(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend), port);
...
void VnSensor::writeAsyncDataOutputType(AsciiAsync ador, uint8_t port, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteAsyncDataOutputType(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), ador, port);
...
uint32_t VnSensor::readAsyncDataOutputFrequency(uint8_t port)
{
  ...
  size_t length = Packet::genReadAsyncDataOutputFrequency(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), port);
...
void VnSensor::writeAsyncDataOutputFrequency(const uint32_t & adof, uint8_t port, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteAsyncDataOutputFrequency(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), adof, port);
...
InsBasicConfigurationRegisterVn200 VnSensor::readInsBasicConfigurationVn200()
{
  ...
  size_t length =
    Packet::genReadInsBasicConfiguration(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  uint8_t scenario;
  uint8_t ahrsAiding;
  response.parseInsBasicConfiguration(&scenario, &ahrsAiding);
...
void VnSensor::writeInsBasicConfigurationVn200(
  InsBasicConfigurationRegisterVn200 & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteInsBasicConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.scenario, fields.ahrsAiding, 0);
...
void VnSensor::writeInsBasicConfigurationVn200(
  Scenario scenario, const uint8_t & ahrsAiding, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteInsBasicConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), scenario, ahrsAiding, 0);
...
InsBasicConfigurationRegisterVn300 VnSensor::readInsBasicConfigurationVn300()
{
  ...
  uint8_t ahrsAiding;
  uint8_t estBaseline;
  response.parseInsBasicConfiguration(&scenario, &ahrsAiding, &estBaseline);
...
void VnSensor::writeInsBasicConfigurationVn300(
  InsBasicConfigurationRegisterVn300 & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteInsBasicConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.scenario, fields.ahrsAiding,
    fields.estBaseline);
...
void VnSensor::writeInsBasicConfigurationVn300(
  Scenario scenario, const uint8_t & ahrsAiding, const uint8_t & estBaseline, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteInsBasicConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), scenario, ahrsAiding, estBaseline);
...
void VnSensor::writeUserTag(const string & tag, bool waitForReply)
{
  ...
  size_t length =
    Packet::genWriteUserTag(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend), tag.c_str());
...
uint32_t VnSensor::readHardwareRevision()
{
  ...
  size_t length =
    Packet::genReadHardwareRevision(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
string VnSensor::readFirmwareVersion()
{
  ...
  size_t length =
    Packet::genReadFirmwareVersion(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
uint32_t VnSensor::readSerialBaudRate()
{
  ...
  size_t length =
    Packet::genReadSerialBaudRate(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeSerialBaudRate(const uint32_t & baudrate, bool waitForReply)
{
  ...
  size_t length =
    Packet::genWriteSerialBaudRate(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend), baudrate);
...
AsciiAsync VnSensor::readAsyncDataOutputType()
{
  ...
  size_t length =
    Packet::genReadAsyncDataOutputType(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeAsyncDataOutputType(AsciiAsync ador, bool waitForReply)
{
  ...
  size_t length =
    Packet::genWriteAsyncDataOutputType(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend), ador);
...
uint32_t VnSensor::readAsyncDataOutputFrequency()
{
  ...
  size_t length =
    Packet::genReadAsyncDataOutputFrequency(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeAsyncDataOutputFrequency(const uint32_t & adof, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteAsyncDataOutputFrequency(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), adof);
...
vec4f VnSensor::readAttitudeQuaternion()
{
  ...
  size_t length =
    Packet::genReadAttitudeQuaternion(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
VnSensor::readQuaternionMagneticAccelerationAndAngularRates()
{
  ...
  size_t length = Packet::genReadQuaternionMagneticAccelerationAndAngularRates(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
vec3f VnSensor::readMagneticMeasurements()
{
  ...
  size_t length =
    Packet::genReadMagneticMeasurements(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
vec3f VnSensor::readAccelerationMeasurements()
{
  ...
  size_t length =
    Packet::genReadAccelerationMeasurements(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
vec3f VnSensor::readAngularRateMeasurements()
{
  ...
  size_t length =
    Packet::genReadAngularRateMeasurements(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
MagneticAccelerationAndAngularRatesRegister VnSensor::readMagneticAccelerationAndAngularRates()
{
  ...
  size_t length = Packet::genReadMagneticAccelerationAndAngularRates(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseMagneticAccelerationAndAngularRates(&reg.mag, &reg.accel, &reg.gyro);
...
MagneticAndGravityReferenceVectorsRegister VnSensor::readMagneticAndGravityReferenceVectors()
{
  ...
  size_t length = Packet::genReadMagneticAndGravityReferenceVectors(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseMagneticAndGravityReferenceVectors(&reg.magRef, &reg.accRef);

  return reg;
}

void VnSensor::writeMagneticAndGravityReferenceVectors(
  MagneticAndGravityReferenceVectorsRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteMagneticAndGravityReferenceVectors(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.magRef, fields.accRef);
...
void VnSensor::writeMagneticAndGravityReferenceVectors(
  const vec3f & magRef, const vec3f & accRef, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteMagneticAndGravityReferenceVectors(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), magRef, accRef);
...
FilterMeasurementsVarianceParametersRegister VnSensor::readFilterMeasurementsVarianceParameters()
{
  ...
  size_t length = Packet::genReadFilterMeasurementsVarianceParameters(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseFilterMeasurementsVarianceParameters(
    &reg.angularWalkVariance, &reg.angularRateVariance, &reg.magneticVariance,
...
void VnSensor::writeFilterMeasurementsVarianceParameters(
  FilterMeasurementsVarianceParametersRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteFilterMeasurementsVarianceParameters(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.angularWalkVariance,
    fields.angularRateVariance, fields.magneticVariance, fields.accelerationVariance);
...
void VnSensor::writeFilterMeasurementsVarianceParameters(
  const float & angularWalkVariance, const vec3f & angularRateVariance,
  const vec3f & magneticVariance, const vec3f & accelerationVariance, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteFilterMeasurementsVarianceParameters(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), angularWalkVariance, angularRateVariance,
...
MagnetometerCompensationRegister VnSensor::readMagnetometerCompensation()
{
  ...
  size_t length =
    Packet::genReadMagnetometerCompensation(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));

  Packet response;
  _pi->transactionNoFinalize(toSend, length, true, &response);

  MagnetometerCompensationRegister reg;
  response.parseMagnetometerCompensation(&reg.c, &reg.b);

  return reg;
}

void VnSensor::writeMagnetometerCompensation(
  MagnetometerCompensationRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteMagnetometerCompensation(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.c, fields.b);
...
void VnSensor::writeMagnetometerCompensation(const mat3f & c, const vec3f & b, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteMagnetometerCompensation(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), c, b);
...
FilterActiveTuningParametersRegister VnSensor::readFilterActiveTuningParameters()
{
  ...
  size_t length = Packet::genReadFilterActiveTuningParameters(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseFilterActiveTuningParameters(
    &reg.magneticDisturbanceGain, &reg.accelerationDisturbanceGain, &reg.magneticDisturbanceMemory,
...
void VnSensor::writeFilterActiveTuningParameters(
  FilterActiveTuningParametersRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteFilterActiveTuningParameters(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.magneticDisturbanceGain,
    fields.accelerationDisturbanceGain, fields.magneticDisturbanceMemory,
    fields.accelerationDisturbanceMemory);
...
void VnSensor::writeFilterActiveTuningParameters(
  const float & magneticDisturbanceGain, const float & accelerationDisturbanceGain,
  const float & magneticDisturbanceMemory, const float & accelerationDisturbanceMemory,
  bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteFilterActiveTuningParameters(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), magneticDisturbanceGain,
    accelerationDisturbanceGain, magneticDisturbanceMemory, accelerationDisturbanceMemory);
...
AccelerationCompensationRegister VnSensor::readAccelerationCompensation()
{
  ...
  size_t length =
    Packet::genReadAccelerationCompensation(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));

  Packet response;
  _pi->transactionNoFinalize(toSend, length, true, &response);

  AccelerationCompensationRegister reg;
  response.parseAccelerationCompensation(&reg.c, &reg.b);

  return reg;
}

void VnSensor::writeAccelerationCompensation(
  AccelerationCompensationRegister & fields, bool waitForReply)
{
  char toSend[256];
...
void VnSensor::writeAccelerationCompensation(const mat3f & c, const vec3f & b, bool waitForReply)
{
  char toSend[256];
...
mat3f VnSensor::readReferenceFrameRotation()
{
  ...
  size_t length =
    Packet::genReadReferenceFrameRotation(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeReferenceFrameRotation(const mat3f & c, bool waitForReply)
{
  ...
  size_t length =
    Packet::genWriteReferenceFrameRotation(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend), c);
...
VnSensor::readYawPitchRollMagneticAccelerationAndAngularRates()
{
  ...
  size_t length = Packet::genReadYawPitchRollMagneticAccelerationAndAngularRates(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseYawPitchRollMagneticAccelerationAndAngularRates(
    &reg.yawPitchRoll, &reg.mag, &reg.accel, &reg.gyro);
...
CommunicationProtocolControlRegister VnSensor::readCommunicationProtocolControl()
{
  ...
  size_t length = Packet::genReadCommunicationProtocolControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeCommunicationProtocolControl(
  CommunicationProtocolControlRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteCommunicationProtocolControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.serialCount, fields.serialStatus,
    fields.spiCount, fields.spiStatus, fields.serialChecksum, fields.spiChecksum, fields.errorMode);
...
void VnSensor::writeCommunicationProtocolControl(
  CountMode serialCount, StatusMode serialStatus, CountMode spiCount, StatusMode spiStatus,
  ChecksumMode serialChecksum, ChecksumMode spiChecksum, ErrorMode errorMode, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteCommunicationProtocolControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), serialCount, serialStatus, spiCount,
    spiStatus, serialChecksum, spiChecksum, errorMode);
...
SynchronizationControlRegister VnSensor::readSynchronizationControl()
{
  ...
  size_t length =
    Packet::genReadSynchronizationControl(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseSynchronizationControl(
    &syncInMode, &syncInEdge, &reg.syncInSkipFactor, &syncOutMode, &syncOutPolarity,
    &reg.syncOutSkipFactor, &reg.syncOutPulseWidth);
...
void VnSensor::writeSynchronizationControl(
  SynchronizationControlRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteSynchronizationControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.syncInMode, fields.syncInEdge,
    fields.syncInSkipFactor, fields.syncOutMode, fields.syncOutPolarity, fields.syncOutSkipFactor,
...
void VnSensor::writeSynchronizationControl(
  SyncInMode syncInMode, SyncInEdge syncInEdge, const uint16_t & syncInSkipFactor,
  SyncOutMode syncOutMode, SyncOutPolarity syncOutPolarity, const uint16_t & syncOutSkipFactor,
  const uint32_t & syncOutPulseWidth, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteSynchronizationControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), syncInMode, syncInEdge, syncInSkipFactor,
    syncOutMode, syncOutPolarity, syncOutSkipFactor, syncOutPulseWidth);
...
SynchronizationStatusRegister VnSensor::readSynchronizationStatus()
{
  ...
  size_t length =
    Packet::genReadSynchronizationStatus(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));

  Packet response;
  _pi->transactionNoFinalize(toSend, length, true, &response);

  SynchronizationStatusRegister reg;
  response.parseSynchronizationStatus(&reg.syncInCount, &reg.syncInTime, &reg.syncOutCount);
...
void VnSensor::writeSynchronizationStatus(SynchronizationStatusRegister & fields, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteSynchronizationStatus(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.syncInCount, fields.syncInTime,
    fields.syncOutCount);
...
void VnSensor::writeSynchronizationStatus(
  const uint32_t & syncInCount, const uint32_t & syncInTime, const uint32_t & syncOutCount,
  bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteSynchronizationStatus(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), syncInCount, syncInTime, syncOutCount);
...
FilterBasicControlRegister VnSensor::readFilterBasicControl()
{
  ...
  size_t length =
    Packet::genReadFilterBasicControl(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseFilterBasicControl(
    &magMode, &extMagMode, &extAccMode, &extGyroMode, &reg.gyroLimit);
...
void VnSensor::writeFilterBasicControl(FilterBasicControlRegister & fields, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteFilterBasicControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.magMode, fields.extMagMode,
    fields.extAccMode, fields.extGyroMode, fields.gyroLimit);
...
void VnSensor::writeFilterBasicControl(
  MagneticMode magMode, ExternalSensorMode extMagMode, ExternalSensorMode extAccMode,
  ExternalSensorMode extGyroMode, const vec3f & gyroLimit, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteFilterBasicControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), magMode, extMagMode, extAccMode,
    extGyroMode, gyroLimit);
...
HeaveConfigurationRegister VnSensor::readHeaveConfiguration()
{
  ...
  size_t length =
    Packet::genReadHeaveConfiguration(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseHeaveConfiguration(
    &reg.initialWavePeriod, &reg.initialWaveAmplitude, &reg.maxWavePeriod, &reg.minWaveAmplitude,
    &reg.delayedHeaveCutoffFreq, &reg.heaveCutoffFreq, &reg.heaveRateCutoffFreq);
...
void VnSensor::writeHeaveConfiguration(HeaveConfigurationRegister & fields, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteHeaveConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.initialWavePeriod,
    fields.initialWaveAmplitude, fields.maxWavePeriod, fields.minWaveAmplitude,
    fields.delayedHeaveCutoffFreq, fields.heaveCutoffFreq, fields.heaveRateCutoffFreq);
...
void VnSensor::writeHeaveConfiguration(
  const float & initialWavePeriod, const float & initialWaveAmplitude, const float & maxWavePeriod,
  const float & minWaveAmplitude, const float & delayedHeaveCutoffFreq,
  const float & heaveCutoffFreq, const float & heaveRateCutoffFreq, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteHeaveConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), initialWavePeriod, initialWaveAmplitude,
    maxWavePeriod, minWaveAmplitude, delayedHeaveCutoffFreq, heaveCutoffFreq, heaveRateCutoffFreq);
...
VpeBasicControlRegister VnSensor::readVpeBasicControl()
{
  ...
  size_t length =
    Packet::genReadVpeBasicControl(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  uint8_t tuningMode;
  response.parseVpeBasicControl(&enable, &headingMode, &filteringMode, &tuningMode);
...
void VnSensor::writeVpeBasicControl(VpeBasicControlRegister & fields, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteVpeBasicControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.enable, fields.headingMode,
    fields.filteringMode, fields.tuningMode);
...
void VnSensor::writeVpeBasicControl(
  VpeEnable enable, HeadingMode headingMode, VpeMode filteringMode, VpeMode tuningMode,
  bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteVpeBasicControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), enable, headingMode, filteringMode,
    tuningMode);
...
VpeMagnetometerBasicTuningRegister VnSensor::readVpeMagnetometerBasicTuning()
{
  ...
  size_t length =
    Packet::genReadVpeMagnetometerBasicTuning(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseVpeMagnetometerBasicTuning(
    &reg.baseTuning, &reg.adaptiveTuning, &reg.adaptiveFiltering);

  return reg;
}

void VnSensor::writeVpeMagnetometerBasicTuning(
  VpeMagnetometerBasicTuningRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteVpeMagnetometerBasicTuning(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.baseTuning, fields.adaptiveTuning,
    fields.adaptiveFiltering);
...
void VnSensor::writeVpeMagnetometerBasicTuning(
  const vec3f & baseTuning, const vec3f & adaptiveTuning, const vec3f & adaptiveFiltering,
  bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteVpeMagnetometerBasicTuning(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), baseTuning, adaptiveTuning,
    adaptiveFiltering);
...
VpeMagnetometerAdvancedTuningRegister VnSensor::readVpeMagnetometerAdvancedTuning()
{
  ...
  size_t length = Packet::genReadVpeMagnetometerAdvancedTuning(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseVpeMagnetometerAdvancedTuning(
    &reg.minFiltering, &reg.maxFiltering, &reg.maxAdaptRate, &reg.disturbanceWindow,
    &reg.maxTuning);
...
void VnSensor::writeVpeMagnetometerAdvancedTuning(
  VpeMagnetometerAdvancedTuningRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteVpeMagnetometerAdvancedTuning(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.minFiltering, fields.maxFiltering,
    fields.maxAdaptRate, fields.disturbanceWindow, fields.maxTuning);
...
void VnSensor::writeVpeMagnetometerAdvancedTuning(
  const vec3f & minFiltering, const vec3f & maxFiltering, const float & maxAdaptRate,
  const float & disturbanceWindow, const float & maxTuning, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteVpeMagnetometerAdvancedTuning(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), minFiltering, maxFiltering, maxAdaptRate,
    disturbanceWindow, maxTuning);
...
VpeAccelerometerBasicTuningRegister VnSensor::readVpeAccelerometerBasicTuning()
{
  ...
  size_t length = Packet::genReadVpeAccelerometerBasicTuning(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeVpeAccelerometerBasicTuning(
  VpeAccelerometerBasicTuningRegister & fields, bool waitForReply)
{
  char toSend[256];
...
VpeAccelerometerAdvancedTuningRegister VnSensor::readVpeAccelerometerAdvancedTuning()
{
  ...
  size_t length = Packet::genReadVpeAccelerometerAdvancedTuning(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeVpeAccelerometerAdvancedTuning(
  VpeAccelerometerAdvancedTuningRegister & fields, bool waitForReply)
{
  char toSend[256];
...
VpeGyroBasicTuningRegister VnSensor::readVpeGyroBasicTuning()
{
  ...
  size_t length =
    Packet::genReadVpeGyroBasicTuning(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));

  Packet response;
  _pi->transactionNoFinalize(toSend, length, true, &response);

  VpeGyroBasicTuningRegister reg;
  response.parseVpeGyroBasicTuning(&reg.angularWalkVariance, &reg.baseTuning, &reg.adaptiveTuning);
...
void VnSensor::writeVpeGyroBasicTuning(VpeGyroBasicTuningRegister & fields, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteVpeGyroBasicTuning(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.angularWalkVariance,
    fields.baseTuning, fields.adaptiveTuning);
...
void VnSensor::writeVpeGyroBasicTuning(
  const vec3f & angularWalkVariance, const vec3f & baseTuning, const vec3f & adaptiveTuning,
  bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteVpeGyroBasicTuning(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), angularWalkVariance, baseTuning,
    adaptiveTuning);
...
vec3f VnSensor::readFilterStartupGyroBias()
{
  ...
  size_t length =
    Packet::genReadFilterStartupGyroBias(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeFilterStartupGyroBias(const vec3f & bias, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteFilterStartupGyroBias(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), bias);
...
MagnetometerCalibrationControlRegister VnSensor::readMagnetometerCalibrationControl()
{
  ...
  size_t length = Packet::genReadMagnetometerCalibrationControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  uint8_t hsiMode;
  uint8_t hsiOutput;
  response.parseMagnetometerCalibrationControl(&hsiMode, &hsiOutput, &reg.convergeRate);
...
void VnSensor::writeMagnetometerCalibrationControl(
  MagnetometerCalibrationControlRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteMagnetometerCalibrationControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.hsiMode, fields.hsiOutput,
    fields.convergeRate);
...
void VnSensor::writeMagnetometerCalibrationControl(
  HsiMode hsiMode, HsiOutput hsiOutput, const uint8_t & convergeRate, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteMagnetometerCalibrationControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), hsiMode, hsiOutput, convergeRate);
...
CalculatedMagnetometerCalibrationRegister VnSensor::readCalculatedMagnetometerCalibration()
{
  ...
  size_t length = Packet::genReadCalculatedMagnetometerCalibration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseCalculatedMagnetometerCalibration(&reg.c, &reg.b);
...
float VnSensor::readIndoorHeadingModeControl()
{
  ...
  size_t length =
    Packet::genReadIndoorHeadingModeControl(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeIndoorHeadingModeControl(const float & maxRateError, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteIndoorHeadingModeControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), maxRateError);
...
vec3f VnSensor::readVelocityCompensationMeasurement()
{
  ...
  size_t length = Packet::genReadVelocityCompensationMeasurement(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeVelocityCompensationMeasurement(const vec3f & velocity, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteVelocityCompensationMeasurement(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), velocity);
...
VelocityCompensationControlRegister VnSensor::readVelocityCompensationControl()
{
  ...
  size_t length = Packet::genReadVelocityCompensationControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  uint8_t mode;
  response.parseVelocityCompensationControl(&mode, &reg.velocityTuning, &reg.rateTuning);
...
void VnSensor::writeVelocityCompensationControl(
  VelocityCompensationControlRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteVelocityCompensationControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.mode, fields.velocityTuning,
    fields.rateTuning);
...
void VnSensor::writeVelocityCompensationControl(
  VelocityCompensationMode mode, const float & velocityTuning, const float & rateTuning,
  bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteVelocityCompensationControl(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), mode, velocityTuning, rateTuning);
...
VelocityCompensationStatusRegister VnSensor::readVelocityCompensationStatus()
{
  ...
  size_t length =
    Packet::genReadVelocityCompensationStatus(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));

  Packet response;
  _pi->transactionNoFinalize(toSend, length, true, &response);

  VelocityCompensationStatusRegister reg;
  response.parseVelocityCompensationStatus(&reg.x, &reg.xDot, &reg.accelOffset, &reg.omega);
...
ImuMeasurementsRegister VnSensor::readImuMeasurements()
{
  ...
  size_t length =
    Packet::genReadImuMeasurements(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));

  Packet response;
  _pi->transactionNoFinalize(toSend, length, true, &response);

  ImuMeasurementsRegister reg;
  response.parseImuMeasurements(&reg.mag, &reg.accel, &reg.gyro, &reg.temp, &reg.pressure);
...
GpsConfigurationRegister VnSensor::readGpsConfiguration()
{
  ...
  size_t length =
    Packet::genReadGpsConfiguration(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  uint8_t rate;
  uint8_t antPow;
  response.parseGpsConfiguration(&mode, &ppsSource, &rate, &antPow);
...
void VnSensor::writeGpsConfiguration(GpsConfigurationRegister & fields, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteGpsConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.mode, fields.ppsSource,
    fields.rate, fields.antPow);
...
void VnSensor::writeGpsConfiguration(GpsMode mode, PpsSource ppsSource, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteGpsConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), mode, ppsSource);
...
void VnSensor::writeGpsConfiguration(
  GpsMode mode, PpsSource ppsSource, GpsRate rate, AntPower antPow, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteGpsConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), mode, ppsSource, rate, antPow);
...
vec3f VnSensor::readGpsAntennaOffset()
{
  ...
  size_t length =
    Packet::genReadGpsAntennaOffset(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeGpsAntennaOffset(const vec3f & position, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteGpsAntennaOffset(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), position);
...
GpsSolutionLlaRegister VnSensor::readGpsSolutionLla()
{
  ...
  size_t length =
    Packet::genReadGpsSolutionLla(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseGpsSolutionLla(
    &reg.time, &reg.week, &gpsFix, &reg.numSats, &reg.lla, &reg.nedVel, &reg.nedAcc, &reg.speedAcc,
    &reg.timeAcc);
...
GpsSolutionEcefRegister VnSensor::readGpsSolutionEcef()
{
  ...
  size_t length =
    Packet::genReadGpsSolutionEcef(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseGpsSolutionEcef(
    &reg.tow, &reg.week, &gpsFix, &reg.numSats, &reg.position, &reg.velocity, &reg.posAcc,
    &reg.speedAcc, &reg.timeAcc);
...
InsSolutionLlaRegister VnSensor::readInsSolutionLla()
{
  ...
  size_t length =
    Packet::genReadInsSolutionLla(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseInsSolutionLla(
    &reg.time, &reg.week, &reg.status, &reg.yawPitchRoll, &reg.position, &reg.nedVel,
    &reg.attUncertainty, &reg.posUncertainty, &reg.velUncertainty);
...
InsSolutionEcefRegister VnSensor::readInsSolutionEcef()
{
  ...
  size_t length =
    Packet::genReadInsSolutionEcef(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseInsSolutionEcef(
    &reg.time, &reg.week, &reg.status, &reg.yawPitchRoll, &reg.position, &reg.velocity,
    &reg.attUncertainty, &reg.posUncertainty, &reg.velUncertainty);
...
InsAdvancedConfigurationRegister VnSensor::readInsAdvancedConfiguration()
{
  ...
  size_t length =
    Packet::genReadInsAdvancedConfiguration(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseInsAdvancedConfiguration(
    &useMag, &usePres, &posAtt, &velAtt, &velBias, &useFoam, &reg.gpsCovType, &reg.velCount,
    &reg.velInit, &reg.moveOrigin, &reg.gpsTimeout, &reg.deltaLimitPos, &reg.deltaLimitVel,
    &reg.minPosUncertainty, &reg.minVelUncertainty);
...
void VnSensor::writeInsAdvancedConfiguration(
  InsAdvancedConfigurationRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteInsAdvancedConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.useMag, fields.usePres,
    fields.posAtt, fields.velAtt, fields.velBias, fields.useFoam, fields.gpsCovType,
    fields.velCount, fields.velInit, fields.moveOrigin, fields.gpsTimeout, fields.deltaLimitPos,
    fields.deltaLimitVel, fields.minPosUncertainty, fields.minVelUncertainty);
...
void VnSensor::writeInsAdvancedConfiguration(
  const uint8_t & useMag, const uint8_t & usePres, const uint8_t & posAtt, const uint8_t & velAtt,
  const uint8_t & velBias, FoamInit useFoam, const uint8_t & gpsCovType, const uint8_t & velCount,
  const float & velInit, const float & moveOrigin, const float & gpsTimeout,
  const float & deltaLimitPos, const float & deltaLimitVel, const float & minPosUncertainty,
  const float & minVelUncertainty, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteInsAdvancedConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), useMag, usePres, posAtt, velAtt, velBias,
    useFoam, gpsCovType, velCount, velInit, moveOrigin, gpsTimeout, deltaLimitPos, deltaLimitVel,
    minPosUncertainty, minVelUncertainty);
...
InsStateLlaRegister VnSensor::readInsStateLla()
{
  ...
  response.parseInsStateLla(
    &reg.yawPitchRoll, &reg.position, &reg.velocity, &reg.accel, &reg.angularRate);
...
StartupFilterBiasEstimateRegister VnSensor::readStartupFilterBiasEstimate()
{
  ...
  size_t length =
    Packet::genReadStartupFilterBiasEstimate(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));

  Packet response;
  _pi->transactionNoFinalize(toSend, length, true, &response);

  StartupFilterBiasEstimateRegister reg;
  response.parseStartupFilterBiasEstimate(&reg.gyroBias, &reg.accelBias, &reg.pressureBias);

  return reg;
}

void VnSensor::writeStartupFilterBiasEstimate(
  StartupFilterBiasEstimateRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteStartupFilterBiasEstimate(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.gyroBias, fields.accelBias,
    fields.pressureBias);
...
void VnSensor::writeStartupFilterBiasEstimate(
  const vec3f & gyroBias, const vec3f & accelBias, const float & pressureBias, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteStartupFilterBiasEstimate(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), gyroBias, accelBias, pressureBias);
...
DeltaThetaAndDeltaVelocityRegister VnSensor::readDeltaThetaAndDeltaVelocity()
{
  ...
  size_t length =
    Packet::genReadDeltaThetaAndDeltaVelocity(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));

  Packet response;
  _pi->transactionNoFinalize(toSend, length, true, &response);

  DeltaThetaAndDeltaVelocityRegister reg;
  response.parseDeltaThetaAndDeltaVelocity(&reg.deltaTime, &reg.deltaTheta, &reg.deltaVelocity);
...
VnSensor::readDeltaThetaAndDeltaVelocityConfiguration()
{
  ...
  size_t length = Packet::genReadDeltaThetaAndDeltaVelocityConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeDeltaThetaAndDeltaVelocityConfiguration(
  DeltaThetaAndDeltaVelocityConfigurationRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteDeltaThetaAndDeltaVelocityConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.integrationFrame,
    fields.gyroCompensation, fields.accelCompensation, fields.earthRateCorrection);
...
void VnSensor::writeDeltaThetaAndDeltaVelocityConfiguration(
  IntegrationFrame integrationFrame, CompensationMode gyroCompensation,
  AccCompensationMode accelCompensation, bool waitForReply)
{
  writeDeltaThetaAndDeltaVelocityConfiguration(
    integrationFrame, gyroCompensation, accelCompensation, protocol::uart::EARTHRATECORR_NONE,
    waitForReply);
}

void VnSensor::writeDeltaThetaAndDeltaVelocityConfiguration(
  IntegrationFrame integrationFrame, CompensationMode gyroCompensation,
  AccCompensationMode accelCompensation, EarthRateCorrection earthRateCorrection, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteDeltaThetaAndDeltaVelocityConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), integrationFrame, gyroCompensation,
    accelCompensation, earthRateCorrection);
...
ReferenceVectorConfigurationRegister VnSensor::readReferenceVectorConfiguration()
{
  ...
  size_t length = Packet::genReadReferenceVectorConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseReferenceVectorConfiguration(
    &useMagModel, &useGravityModel, &reg.recalcThreshold, &reg.year, &reg.position);
...
void VnSensor::writeReferenceVectorConfiguration(
  ReferenceVectorConfigurationRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteReferenceVectorConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.useMagModel,
    fields.useGravityModel, fields.recalcThreshold, fields.year, fields.position);
...
void VnSensor::writeReferenceVectorConfiguration(
  const uint8_t & useMagModel, const uint8_t & useGravityModel, const uint32_t & recalcThreshold,
  const float & year, const vec3d & position, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteReferenceVectorConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), useMagModel, useGravityModel,
    recalcThreshold, year, position);
...
GyroCompensationRegister VnSensor::readGyroCompensation()
{
  ...
  size_t length =
    Packet::genReadGyroCompensation(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));

  Packet response;
  _pi->transactionNoFinalize(toSend, length, true, &response);

  GyroCompensationRegister reg;
  response.parseGyroCompensation(&reg.c, &reg.b);
...
void VnSensor::writeGyroCompensation(const mat3f & c, const vec3f & b, bool waitForReply)
{
  char toSend[256];

  size_t length =
    Packet::genWriteGyroCompensation(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend), c, b);
...
ImuFilteringConfigurationRegister VnSensor::readImuFilteringConfiguration()
{
  ...
  size_t length =
    Packet::genReadImuFilteringConfiguration(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseImuFilteringConfiguration(
    &reg.magWindowSize, &reg.accelWindowSize, &reg.gyroWindowSize, &reg.tempWindowSize,
    &reg.presWindowSize, &magFilterMode, &accelFilterMode, &gyroFilterMode, &tempFilterMode,
    &presFilterMode);
...
void VnSensor::writeImuFilteringConfiguration(
  ImuFilteringConfigurationRegister & fields, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteImuFilteringConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.magWindowSize,
    fields.accelWindowSize, fields.gyroWindowSize, fields.tempWindowSize, fields.presWindowSize,
    fields.magFilterMode, fields.accelFilterMode, fields.gyroFilterMode, fields.tempFilterMode,
    fields.presFilterMode);
...
void VnSensor::writeImuFilteringConfiguration(
  const uint16_t & magWindowSize, const uint16_t & accelWindowSize, const uint16_t & gyroWindowSize,
  const uint16_t & tempWindowSize, const uint16_t & presWindowSize, FilterMode magFilterMode,
  FilterMode accelFilterMode, FilterMode gyroFilterMode, FilterMode tempFilterMode,
  FilterMode presFilterMode, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteImuFilteringConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), magWindowSize, accelWindowSize,
    gyroWindowSize, tempWindowSize, presWindowSize, magFilterMode, accelFilterMode, gyroFilterMode,
    tempFilterMode, presFilterMode);
...
GpsCompassBaselineRegister VnSensor::readGpsCompassBaseline()
{
  ...
  size_t length =
    Packet::genReadGpsCompassBaseline(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));

  Packet response;
  _pi->transactionNoFinalize(toSend, length, true, &response);

  GpsCompassBaselineRegister reg;
  response.parseGpsCompassBaseline(&reg.position, &reg.uncertainty);
...
void VnSensor::writeGpsCompassBaseline(GpsCompassBaselineRegister & fields, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteGpsCompassBaseline(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.position, fields.uncertainty);
...
void VnSensor::writeGpsCompassBaseline(
  const vec3f & position, const vec3f & uncertainty, bool waitForReply)
{
  char toSend[256];
...
GpsCompassEstimatedBaselineRegister VnSensor::readGpsCompassEstimatedBaseline()
{
  ...
  size_t length = Packet::genReadGpsCompassEstimatedBaseline(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseGpsCompassEstimatedBaseline(
    &estBaselineUsed, &reg.numMeas, &reg.position, &reg.uncertainty);
...
ImuRateConfigurationRegister VnSensor::readImuRateConfiguration()
{
  ...
  size_t length =
    Packet::genReadImuRateConfiguration(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseImuRateConfiguration(
    &reg.imuRate, &reg.navDivisor, &reg.filterTargetRate, &reg.filterMinRate);
...
void VnSensor::writeImuRateConfiguration(ImuRateConfigurationRegister & fields, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteImuRateConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.imuRate, fields.navDivisor,
    fields.filterTargetRate, fields.filterMinRate);
...
void VnSensor::writeImuRateConfiguration(
  const uint16_t & imuRate, const uint16_t & navDivisor, const float & filterTargetRate,
  const float & filterMinRate, bool waitForReply)
{
  char toSend[256];

  size_t length = Packet::genWriteImuRateConfiguration(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), imuRate, navDivisor, filterTargetRate,
    filterMinRate);
...
VnSensor::readYawPitchRollTrueBodyAccelerationAndAngularRates()
{
  ...
  size_t length = Packet::genReadYawPitchRollTrueBodyAccelerationAndAngularRates(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseYawPitchRollTrueBodyAccelerationAndAngularRates(
    &reg.yawPitchRoll, &reg.bodyAccel, &reg.gyro);
...
VnSensor::readYawPitchRollTrueInertialAccelerationAndAngularRates()
{
  ...
  size_t length = Packet::genReadYawPitchRollTrueInertialAccelerationAndAngularRates(
    _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseYawPitchRollTrueInertialAccelerationAndAngularRates(
    &reg.yawPitchRoll, &reg.inertialAccel, &reg.gyro);
...
void VnSensor::switchProcessors(VnProcessorType processor, std::string model, std::string firmware)
{
  ...
  if (model.find("VN-300") != std::string::npos) {
    ...
    (model.find("VN-110") != std::string::npos) || (model.find("VN-210") != std::string::npos) ||
    (model.find("VN-310") != std::string::npos)) {
    switch (processor) {
      ...
      case VNPROCESSOR_GPS:
        if (
          (model.find("VN-310") != std::string::npos) ||
          (model.find("VN-210E") != std::string::npos)) {
          switchCmd = "$VNSBL,1";
        }
        break;
...
void VnSensor::writeFirmwareUpdateRecord(const string & record, bool waitForReply)
{
  ...
  size_t length = Packet::genWriteFirmwareUpdateRecord(
    ERRORDETECTIONMODE_CRC, toSend, sizeof(toSend), record.c_str());

  Packet response;
RESEND:
  _pi->_filteringBootloaderResponses =
    true; /*Enable Bootloader Filtering just before sending the bootloader command */
...
void VnSensor::stopRequest() { _pi->port->stopThread(); }

bool VnSensor::threadStopped() { return _pi->port->threadStopped(); }

void VnSensor::unregisterListners() { _pi->port->unregisterDataReceivedHandler(); }
