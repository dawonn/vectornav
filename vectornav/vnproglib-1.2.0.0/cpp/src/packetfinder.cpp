...
{
  return lhs.possibleStartIndex == rhs.possibleStartIndex &&
         lhs.groupsPresentFound == rhs.groupsPresentFound &&
         lhs.groupsPresent == rhs.groupsPresent &&
         lhs.numOfBytesRemainingToHaveAllGroupFields ==
           rhs.numOfBytesRemainingToHaveAllGroupFields &&
...
struct PacketFinder::Impl
{
  ...
  size_t
    _runningDataIndex;  // Used for correlating raw data with where the packet was found for the end user.
  ...
  void dataReceived(uint8_t data[], size_t length, bool bootloaderFilter, TimeStamp timestamp)
  {
    ...
    for (size_t i = 0; i < length; i++, _runningDataIndex++) {
      if (
        data[i] == AsciiStartChar ||
        (bootloaderFilter &&
         (!_asciiOnDeck.currentlyBuildingAsciiPacket && data[i] == BootloaderVersionStartChar))) {
        _asciiOnDeck.reset();
        ...
        ((bootloaderFilter && _asciiOnDeck.currentlyBuildingAsciiPacket) ||
         (!bootloaderFilter && _asciiOnDeck.asciiEndChar1Found)) &&
        data[i] == AsciiEndChar2) {
        ...
        if (p.isValid()) dispatchPacket(p, runningIndexOfPacketStart, _asciiOnDeck.timeFound);
      ...
      for (list<BinaryTracker>::iterator it = _binaryOnDeck.begin(); it != _binaryOnDeck.end();
           ++it) {
        BinaryTracker & ez = (*it);
        ...
        if (ez.numOfBytesRemainingToHaveAllGroupFields != 0) {
          ...
          if (ez.numOfBytesRemainingToHaveAllGroupFields == 0) {
            ...
            if (ez.startFoundInProvidedDataBuffer) {
              ...
              remainingBytesForCompletePacket =
                Packet::computeBinaryPacketLength(
                  reinterpret_cast<char *>(data) + ez.possibleStartIndex) -
                headerLength;
            } else {
              ...
              size_t headerLength =
                _bufferAppendLocation - ez.possibleStartIndex + numOfBytesToCopyIntoReceiveBuffer;

              if (_bufferAppendLocation + numOfBytesToCopyIntoReceiveBuffer < _bufferSize) {
                std::memcpy(
                  _buffer + _bufferAppendLocation, data, numOfBytesToCopyIntoReceiveBuffer);

                remainingBytesForCompletePacket =
                  Packet::computeBinaryPacketLength(
                    reinterpret_cast<char *>(_buffer) + ez.possibleStartIndex) -
                  headerLength;
              } else {
    ...
    if (_asciiOnDeck.currentlyBuildingAsciiPacket && asciiStartFoundInProvidedBuffer) {
      if (_asciiOnDeck.possibleStartOfPacketIndex < dataIndexToStartCopyingFrom) {
        binaryDataMoveOverIndexAdjustment -=
          binaryDataMoveOverIndexAdjustment - _asciiOnDeck.possibleStartOfPacketIndex;
      ...
      _asciiOnDeck.possibleStartOfPacketIndex = _bufferAppendLocation +
                                                _asciiOnDeck.possibleStartOfPacketIndex -
                                                dataIndexToStartCopyingFrom;
    }
    ...
    for (list<BinaryTracker>::iterator it = _binaryOnDeck.begin(); it != _binaryOnDeck.end();
      ...
      if ((*it).startFoundInProvidedDataBuffer) {
        ...
        (*it).possibleStartIndex =
          (*it).possibleStartIndex - binaryDataMoveOverIndexAdjustment + _bufferAppendLocation;
      }
    }
  ...
  void dispatchPacket(Packet & packet, size_t runningDataIndexAtPacketStart, TimeStamp timestamp)
  {
    if (_possiblePacketFoundHandler != NULL) {
      _possiblePacketFoundHandler(
        _possiblePacketFoundUserData, packet, runningDataIndexAtPacketStart, timestamp);
    }
  }
};
...
PacketFinder::PacketFinder() : _pi(new Impl(this)) {}
...
PacketFinder::~PacketFinder() { delete _pi; }
...
void PacketFinder::processReceivedData(
  char data[], size_t length, bool bootloaderFilter, TimeStamp timestamp)
{
...
void PacketFinder::registerPossiblePacketFoundHandler(
...
boost::python::object * PacketFinder::register_packet_found_handler(
  PyObject * callable /*boost::python::object* callable*/)
{
