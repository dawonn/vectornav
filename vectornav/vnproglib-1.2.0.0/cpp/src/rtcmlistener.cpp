...
void rtcmlistener::communicationThreadWithRTCMServer()
{
  ...
  if (iResult != NO_ERROR) {
    printf(
      "RTCM_Server::CommunicationThreadWithRTCMServer: sockInit failed with error: %d\n", iResult);
    return;
  }
  ...
  if (iResult != 0) {
    printf(
      "RTCM_Server::CommunicationThreadWithRTCMServer: getaddrinfo failed with error: %d\n",
      iResult);
    sockQuit();
    return;
  }
  ...
  for (ptr = result; ptr != NULL; ptr = ptr->ai_next) {
    ...
    if (!sockValid(serverSocket)) {
      printf(
        "RTCM_Server::CommunicationThreadWithRTCMServer: socket failed with error: %d\n",
        GETSOCKETERROR());
      sockQuit();
      return;
    }
    ...
    if (bytesReceived > 0) {
      ...
      while (index < bytesReceived - 2) {
        ...
        if (buffer[index] == (char)0xD3) {
          ...
          int packetSize =
            (((buffer[index + 1] & 0xFF) & 0x03) * 256) + (buffer[index + 2] & 0xFF) + 6;
...
void rtcmlistener::setServerHost(std::string host) { serverHost = host; }

void rtcmlistener::setServerPort(std::string port) { serverPort = port; }
