...
#error "Unknown System"
  ...
  {
    if (checkAndToggleIsOpenFlag) ensureOpened();

    StopSerialPortNotificationsThread();

#if _WIN32

    if (!CloseHandle(SerialPortHandle)) throw unknown_error();

#elif __linux__ || __APPLE__ || __CYGWIN__ || __QNXNTO__

    if (::close(SerialPortHandle) == -1) throw unknown_error();

#else
#error "Unknown System"
#endif

    if (checkAndToggleIsOpenFlag) IsOpen = false;
  }
...
#error "Unknown System"
  ...
  void HandleSerialPortNotifications()
  {
    ...
    SetCommMask(SerialPortHandle, EV_RXCHAR | EV_ERR | EV_RX80FULL);
  ...
  IgnoreError:
    ...
    try {
      ...
      while (ContinueHandlingSerialPortEvents) {
        ...
        BOOL result = WaitCommEvent(SerialPortHandle, &mask, &overlapped);
      ...
      KeepWaiting:
        ...
        DWORD waitResult = WaitForSingleObject(overlapped.hEvent, WaitTimeForSerialPortReadsInMs);

        if (!ContinueHandlingSerialPortEvents) break;

        if (waitResult == WAIT_TIMEOUT) goto KeepWaiting;

        if (waitResult != WAIT_OBJECT_0)
          // Something unexpected happened.
          break;

        if (!GetOverlappedResult(SerialPortHandle, &overlapped, &temp, TRUE)) {
        ...
        if (mask & EV_ERR) {
          ...
          if (!ClearCommError(SerialPortHandle, &spErrors, &comStat)) {
            // Something unexpected happened.
            break;
          }
        ...
        error = select(SerialPortHandle + 1, &readfs, NULL, NULL, &readWaitTime);
        ...
        if (!FD_ISSET(SerialPortHandle, &readfs)) continue;
...
#error "Unknown System"
    ...
    if (userUnpluggedUsbCable) closeAfterUsbCableUnplugged();
  }
  ...
  void StartSerialPortNotificationsThread()
  {
    ...
    pSerialPortEventsThread = Thread::startNew(HandleSerialPortNotifications, this);
  }
...

  {
    ...
    BackReference->read(buffer, NumberOfBytesToPurgeOnOpeningSerialPort, numOfBytesRead);
  }
  ...
  void ensureOpened()
  {
    if (!IsOpen) throw invalid_operation("Port is not opened.");
  }

  void ensureClosed()
  {
    if (IsOpen) throw invalid_operation("Port is not closed.");
  }

  void open(bool checkAndToggleIsOpenFlag = true)
  {
    if (checkAndToggleIsOpenFlag) ensureClosed();
    ...
    SerialPortHandle = CreateFileA(
      fullPortName.c_str(), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
...
#pragma warning(pop)
    ...
    if (!GetCommState(SerialPortHandle, &config)) {
      ...
      if (error != ERROR_OPERATION_ABORTED) throw unknown_error();

      // Try clearing this error.
      DWORD errors;
      if (!ClearCommError(SerialPortHandle, &errors, NULL)) throw unknown_error();

      // Retry the operation.
      if (!GetCommState(SerialPortHandle, &config)) throw unknown_error();
    }
    ...
    if (!SetCommState(SerialPortHandle, &config)) {
      ...
      // Retry the operation.
      if (!SetCommState(SerialPortHandle, &config)) throw unknown_error();
    }
    ...
    if (!SetCommTimeouts(SerialPortHandle, &comTimeOut)) {
      ...
      // Retry the operation.
      if (!SetCommTimeouts(SerialPortHandle, &comTimeOut)) throw unknown_error();
    }
...
#error "Unknown System"
    ...
    memset(&portSettings, 0, sizeof(termios));
...
// Set baudrate, 8n1, no modem control, and enable receiving characters.
    ...
    if (tcflush(portFd, TCIFLUSH) != 0) throw unknown_error();

    if (tcsetattr(portFd, TCSANOW, &portSettings) != 0) throw unknown_error();
...
#error "Unknown System"
    ...
    if (checkAndToggleIsOpenFlag) IsOpen = true;

    if (PurgeFirstDataBytesWhenSerialPortIsFirstOpened) PurgeFirstDataBytesFromSerialPort();
...
SerialPort::SerialPort(const string & portName, uint32_t baudrate) : _pi(new Impl(this))
{
...
bool SerialPort_isFtdiUsbSerialPort(string portName)
{
  ...
  if (deviceInfoSet == INVALID_HANDLE_VALUE) throw unknown_error();
  ...
  while (SetupDiEnumDeviceInfo(deviceInfoSet, curDevId, &deviceData)) {
    curDevId++;
    TCHAR friendlyName[0x100];

    if (!SetupDiGetDeviceRegistryProperty(
          deviceInfoSet, &deviceData, SPDRP_FRIENDLYNAME, NULL, (PBYTE)friendlyName,
    ...
    TCHAR mfgName[0x100];
    if (!SetupDiGetDeviceRegistryProperty(
          deviceInfoSet, &deviceData, SPDRP_MFG, NULL, (PBYTE)mfgName, sizeof(mfgName), NULL)) {
      SetupDiDestroyDeviceInfoList(deviceInfoSet);
...
HKEY SerialPort_getRegistryKeyForActiveFtdiPort(string portName, bool isReadOnly)
{
  ...
  while (SetupDiEnumDeviceInfo(deviceInfoSet, curDevId, &deviceData)) {
    ...
    if (!SetupDiGetDeviceInstanceId(
          deviceInfoSet, &deviceData, deviceInstanceId, sizeof(deviceInstanceId), NULL)) {
      SetupDiDestroyDeviceInfoList(deviceInfoSet);
...
#if VN_HAVE_SECURE_CRT
  _stprintf_s(
    ftdiKeyPath, sizeof(ftdiKeyPath),
    TEXT("SYSTEM\\CurrentControlSet\\Enum\\%s\\Device Parameters"), deviceInstanceId);
#else
  _stprintf(
    ftdiKeyPath, TEXT("SYSTEM\\CurrentControlSet\\Enum\\%s\\Device Parameters"), deviceInstanceId);
#endif

  REGSAM accessType = isReadOnly ? KEY_READ : KEY_READ | KEY_SET_VALUE;

  DWORD result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, ftdiKeyPath, 0, accessType, &ftdiKey);

  if (result == ERROR_ACCESS_DENIED) throw permission_denied();

  if (result != ERROR_SUCCESS) throw unknown_error();
...
bool SerialPort::determineIfPortIsOptimized(string portName)
{
  ...
  if (
    RegQueryValueEx(
      ftdiKey, TEXT("LatencyTimer"), NULL, NULL, (LPBYTE)&latencyTimerValue,
...
void SerialPort::optimizePort(string portName)
{
  ...
  if (
    RegSetValueEx(
      ftdiKey, TEXT("LatencyTimer"), 0, REG_DWORD, (PBYTE)&latencyTimerValue, sizeof(DWORD)) !=
    ERROR_SUCCESS) {
    throw unknown_error();
  }
...
std::vector<std::string> SerialPort::getPortNames()
{
  ...
  error = RegOpenKeyEx(
    HKEY_LOCAL_MACHINE, TEXT("HARDWARE\\DEVICEMAP\\SERIALCOMM"), 0, KEY_READ, &serialCommKey);

  if (ERROR_SUCCESS == error) {
    error = RegQueryInfoKey(
      serialCommKey, NULL, NULL, NULL, &numOfSubkeys, NULL, NULL, &numOfValues, NULL, NULL, NULL,
      NULL);
  }
  ...
  if (ERROR_SUCCESS == error) {
    for (size_t i = 0; i < numOfValues; i++) {
      ...
      error = RegEnumValue(serialCommKey, i, value, &capacity, NULL, NULL, (LPBYTE)data, &dataSize);

      if (error != ERROR_SUCCESS) throw unknown_error();

#ifdef UNICODE

      char converted[0x100];
      int convertResult =
        WideCharToMultiByte(CP_ACP, 0, data, dataSize, converted, sizeof(converted), NULL, NULL);

      if (convertResult == 0) throw unknown_error();
...
#error "Unknown System"
  ...
  if ((dp = opendir("/dev")) == NULL) throw unknown_error();

  while ((dirp = readdir(dp)) != NULL) {
    if (strstr(dirp->d_name, "tty.usbserial") != NULL) comPorts.push_back(string(dirp->d_name));
  }
...
void SerialPort::open() { _pi->open(); }

void SerialPort::close() { _pi->close(); }

bool SerialPort::isOpen() { return _pi->IsOpen; }

SerialPort::StopBits SerialPort::stopBits() { return _pi->stopBits; }
...
void SerialPort::write(const char data[], size_t length)
{
  ...
  result = WriteFile(_pi->SerialPortHandle, data, length, NULL, &overlapped);
  ...
  result = GetOverlappedResult(
    _pi->SerialPortHandle, &overlapped, reinterpret_cast<LPDWORD>(&numOfBytesWritten), true);
  ...
  if (!result) throw unknown_error();

#elif __linux__ || __APPLE__ || __CYGWIN__ || __QNXNTO__

  ssize_t numOfBytesWritten = ::write(_pi->SerialPortHandle, data, length);

  if (numOfBytesWritten == -1) throw unknown_error();
...
void SerialPort::read(char dataBuffer[], size_t numOfBytesToRead, size_t & numOfBytesActuallyRead)
{
  ...
  BOOL result = ReadFile(_pi->SerialPortHandle, dataBuffer, numOfBytesToRead, NULL, &overlapped);
  ...
  result = GetOverlappedResult(
    _pi->SerialPortHandle, &overlapped, reinterpret_cast<LPDWORD>(&numOfBytesActuallyRead), true);
  ...
  int result = ::read(_pi->SerialPortHandle, dataBuffer, numOfBytesToRead);

  if (result == -1) throw unknown_error();
...
{
  if (_pi->_dataReceivedHandler != NULL) throw invalid_operation();
...
uint32_t SerialPort::baudrate() { return _pi->Baudrate; }

std::string SerialPort::port() { return _pi->PortName; }
...
void SerialPort::stopThread() { _pi->externalStop(); }

bool SerialPort::threadStopped() { return _pi->ThreadStopped; }

void SerialPort::resumeThread() { _pi->externalStopFinished(); }
