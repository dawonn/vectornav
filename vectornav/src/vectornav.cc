...
void Vectornav::reconnect_timer()
{
  ...
  // Try to reconnect
  try {
    if (vs_->isConnected()) vs_->disconnect();
...
void Vectornav::execute_cal(const std::shared_ptr<MagCalGH> goal_handle)
{
  ...
  vn::sensors::MagnetometerCalibrationControlRegister magControl = {
    vn::protocol::uart::HsiMode::HSIMODE_RESET, vn::protocol::uart::HsiOutput::HSIOUTPUT_NOONBOARD,
  ...
  if (hsiMode.hsiMode != vn::protocol::uart::HsiMode::HSIMODE_RUN) {
    RCLCPP_ERROR_STREAM(
      get_logger(),
      "IMU HSI mode did not return to run after reset! returned mode: " << hsiMode.hsiMode);
    goal_handle->abort(result);
    return;
  }
  ...
  while (calSamples < 1000 && !goal_handle->is_canceling()) {
    ...
    if (calSamples > 20) {
      ...
      if (std::all_of(
            feedbackMsg->curr_avg_dev.cbegin(), feedbackMsg->curr_avg_dev.cend(),
  ...
  try {
    ...
    RCLCPP_FATAL_STREAM(
      get_logger(),
      "Failed to reset IMU to configuration, DRIVER MUST BE RESTARTED\n ERROR: " << e.what());
  } catch (...) {
    RCLCPP_FATAL(
      get_logger(),
      "Failed to reset IMU to configuration, DRIVER MUST BE RESTARTED\n ERROR: unknown");
